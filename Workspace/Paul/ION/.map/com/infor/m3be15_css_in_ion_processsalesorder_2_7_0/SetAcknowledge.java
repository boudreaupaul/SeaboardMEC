...
public class SetAcknowledge extends Globals {
	...
	private void setAcknowledge() throws Throwable {
		// Set tenant ID
    	oTenantID = iCONO.trim();
    	// Set accounting entity ID
    	if (!IONToolbox.isEmpty(iDIVI)) {
    		oAccountingEntityID = iDIVI.trim();
    	}
    	// Set location ID
    	oLocationID = IONToolbox.createLocationID(iFACI, iWHLO);
